...
\begin{proof}
\noindent 
Let $f: \mathbb{R} - \{0\} \rightarrow \mathbb{R}$ be defined by $f(x)=\frac{x+1}{x}$. If $x$ is an arbitrary element in the domain of $f$, then $x\in \mathbb{R}$ and $x \notin \{0\}$, by definition of set difference. It follows that all elements in the domain of $f$ are real numbers and non-zero.
\newline
\textbf{Injective:} 
Let $x_1,x_2$ be in the domain of $f$. Suppose $f(x_1) = f(x_2)$. Since $f(x) = \frac{x+1}{x}$, using the function definition of $f$ on $f(x_1)$ and $f(x_2)$ yields:
...
Let $y$ be an element in the codomain of $f$. Consider $y = 1$. 
...
\begin{align*}
    f(x) &= \frac{x+1}{x}\\
         ...
         &= 1+\frac{1}{x}.
\end{align*}
Since $x \neq 0$, we have $\frac{x}{x} = 1$, and since the quotient of two non-zero numbers is non-zero, $\frac{1}{x} \neq 0$. It follows that $f(x) = 1 + \frac{1}{x} \neq 1$ for any $x \in \mathbb{R}-\{0\}$. Thus $y = 1$ is in the codomain of $f$, but there is no $x$ in the domain where $f(x)=1$. Therefore, by definition of surjectivity, $f$ is not surjective. We've shown that the function $f$ is injective, but not surjective, so the theorem holds.
\end{proof}
\newpage
