...
\medskip
\textbf{Reflexive:} Let $a \in \mathbb{Z}.$ Since $a^2 - a^2 = 0 = 4(0)$, and $0 \in \mathbb{Z}$, it follows by definition of divides that $4 \mid (a^2-a^2)$. Thus, $aRa$ by definition of $R$, so the relation $R$ is reflexive.

\medskip
\textbf{Symmetric:} Let $a,b \in \mathbb{Z}$. Suppose $aRb$. By definition of $R$, $4 \mid (a^2-b^2)$. By definition of divides, $a^2-b^2=4k$ for some integer $k$. Multiplying both sides of the equation by $-1$ yields $b^2-a^2 = 4(-k)$. Since $k$ is an integer, $-k$ is also an integer. Thus, by definition of divides, $4 \mid (b^2 - a^2)$. Therefore, $bRa$ by definition of $R$, proving that $R$ is symmetric.

\medskip
\textbf{Transitive:} Let $a,b,c \in \mathbb{Z}$. Suppose $aRb$ and $bRc$. By definition of $R$, there exist integers $k$ and $n$ such that $a^2-b^2=4k$ and $b^2-c^2=4n$. Adding these two equations yields:
\[
...
\begin{center}(Exam 3 Review $\#7$)\end{center}
\textbf{Theorem:} The function $f: \mathbb{R}-\{0\} \rightarrow \mathbb{R}$ defined by $f(x) = \frac{x+1}{x}$ is injective, but not surjective.
\begin{proof}
Let $f: \mathbb{R} - \{0\} \rightarrow \mathbb{R}$ be defined by $f(x)=\frac{x+1}{x}$. Let $x\in \mathbb{R}-\{0\}$ be an arbitrary element in the domain of $f$, so by definition of set difference, $x\in \mathbb{R}$ and $x \notin \{0\}$, which implies $x\ne0$.
\medskip
\newline
\textbf{Injective:} 
...
\medskip
\textbf{Surjective:} 
Consider $y = 1$, which is an element of the codomain $\mathbb{R}$. By definition of $f$, we have:
\begin{align*}
    f(x) &= \frac{x+1}{x}\\
         &= \frac{x}{x}+\frac{1}{x}\\
         &= 1+\frac{1}{x}.
\end{align*}
Because $x \neq 0$, we have $\frac{x}{x} = 1$, and since the quotient of non-zero numbers is non-zero, $\frac{1}{x} \neq 0$. It follows that $f(x) = 1 + \frac{1}{x} \neq 1$ for any $x \in \mathbb{R}-\{0\}$. Thus $y = 1$ is in the codomain of $f$, but there is no $x$ in the domain where $f(x)=1$. Therefore, by definition of surjectivity, $f$ is not surjective. 

We've shown that the function $f$ is injective, but not surjective, so the theorem holds.
\end{proof}
\newpage
...
\begin{align*}
    ...
    &= 2|x-2||x+2| 
\end{align*}
Because we have established that $|x+2| < 5$, and because $|x-2| < \delta \le \frac{\epsilon}{10}$, we can apply these bounds:
\begin{align*}
...
\begin{proof}
Let $x \in \mathbb{R}^+$. Since $x$ is a real number, $x-1$ is also a real number. A fundamental property of the real numbers is that the square of any real number is non-negative. Therefore, we establish that:
\[
    (x-1)^2 \ge 0
\]
...
\[
    x^2 + 1 \ge 2x
\]
Because $x \in \mathbb{R}^+$, we know that $x$ is strictly positive. Therefore, we can divide both sides of the inequality by $x$ without encountering zero-division or reversing the direction of the inequality sign. Doing so results in:
\begin{align*}
